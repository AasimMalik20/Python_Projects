...
#-------------------#
#This block of code is for py installer#
def resource_path(relative_path):
    """ Get absolute path to resource, works for dev and for PyInstaller """
    try:
        #pyinstaller creates a temp folder and saves it in _MEIPASS
...
def reset_timer():
    ...
    canvas.itemconfig(pomodoro_app_timer, text='00:00')
    reps = 0
...
def timer(count):
    ...
    canvas.itemconfig(pomodoro_app_timer, text=f'{minutes}:{seconds}')
    if count > 0:
...
picture = PhotoImage(file=resource_path('tomato.png'))
canvas.create_image(100, 112, image=picture)
pomodoro_app_timer = canvas.create_text(100, 130, text='00:00', fill='white', font=(FONT_NAME, 28, 'bold'))
